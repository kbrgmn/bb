<template>
    <div>
        <div class="mt-2 sm:mt-0">
            <div class="max-w-2xl justify-center rounded-md border-2 border-dashed border-gray-400 px-6 py-2.5" v-bind="getRootProps()">
                <div class="space-y-1 text-center grid grid-cols-2">
                    <DocumentPlusIcon class="mx-auto h-12 w-12 text-gray-400" />
                    <!--                    <div v-if="isDragActive" class="rounded-md bg-white font-medium text-indigo-600 h-full">Loslassen um Dateien abzulegen...</div>-->
                    <div class="text-sm text-gray-600">
                        <label for="file-upload">
                            <button
                                v-if="!isDragActive"
                                accesskey="a"
                                @click="fileChooserOpen"
                                class="h-full px-2 relative cursor-pointer rounded-md bg-white font-medium text-indigo-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 hover:text-indigo-500"
                            >
                                Klicken um Dokumente <u>a</u>uszuwählen
                            </button>
                            <button
                                v-else
                                class="h-full px-2 relative cursor-pointer rounded-md bg-white font-medium text-indigo-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 hover:text-indigo-500"
                            >
                                Loslassen um Dateien abzulegen...
                            </button>
                    ...
                    <div v-if="!isDragActive" class="text-sm text-gray-600">
                        <p class="hidden sm:block my-auto">oder per <span class="font-semibold">drag & drop</span> in dieses Feld ziehen.</p>
                    </div>
                    <div v-else class="text-sm font-semibold">Drag & Drop</div>
                </div>
            </div>
        </div>

        <div class="text-1xl mt-3.5 text-gray-800 sm:text-xl flex flex-row justify-between">
            <h2>
                Dateiliste<span v-if="!requestPending"> ({{ files.length }})</span>:
            </h2>

            <span v-if="uploadingCount > 0" class="text-xs sm:text-lg inline-flex items-center">
                <InlineLoadingCircle class="pr-1.5" size="4" />
                <h3>Upload - {{ uploadingCount }} verbleibend...</h3>
            </span>
            <button v-else-if="files.length > 0" @click="sortFileList" class="mr-1 px-2 rounded-lg text-base bg-white mb-1.5 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                Sortieren
            </button>
        </div>

        <div class="border-gray-300 px-2 sm:px-4 border-2 overflow-x-scroll" style="min-height: 50vh; max-height: 54vh">
            <ul class="divide-y divide-gray-200 mt-1" role="list">
                ...
                <li v-for="file in files" :key="file.id" class="flex flex-col sm:flex-row justify-between gap-x-6 py-2 hover:bg-gray-100 px-2">
                    <div class="flex justify-around sm:justify-start min-w-0 gap-x-4 items-center">
                        <h3 class="truncate text-sm font-semibold leading-6 text-gray-900">{{ file.fileName }}</h3>

                        <div v-if="file['state'] === 'PROCESSING'" class="bg-blue-400 pl-2 pr-2.5">
                            <LoadingCircle>Verarbeite...</LoadingCircle>
                        </div>
                        <div v-else-if="file['state'] === 'READY'" class="bg-green-50 pl-2 pr-2.5">
                            <p class="truncate text-sm text-green-800 flex items-center py-0.5">
                                <CheckIcon class="h-5 mr-1" />
                                {{ file.title }}
                            </p>
                        </div>
                        <div v-else-if="file['state'] === 'CORRECTION'" class="bg-yellow-50 pl-2 pr-2.5">
                            <p class="truncate text-sm text-yellow-600 flex items-center py-0.5">
                                <ExclamationTriangleIcon class="h-5 mr-1" />
                                {{ file.title }}
                            </p>
                        </div>
                        <div v-else-if="file['state'] === 'FAILURE'" class="bg-red-50 pl-2 pr-2.5">
                            <p class="truncate text-sm text-red-800 flex items-center py-0.5">
                    ...
                    <div class="flex divide-x divide-gray-200 mx-auto sm:mx-0 pr-0 sm:pr-2">
                        <div class="pr-3">
                            <div class="flex flex-1 w-28">
                                <NuxtLink
                                    v-if="file['state'] !== 'LOADING'"
                                    :to="`/org/${orgId}/ocr-import/sessions/${sessionId}/edit/${file.id}`"
                                    class="relative inline-flex w-0 flex-1 items-center justify-center gap-x-1.5 rounded-md border border-transparent py-1 text-sm font-semibold group text-gray-900 hover:bg-blue-400 hover:text-white hover:scale-110"
                                >
                        ...
                        <div class="pl-3">
                            <div class="flex flex-1 w-24">
                                <CardActionButton
                                    :icon="TrashIcon"
                                    action-text="Löschen..."
                                    class="py-1 text-gray-900 hover:bg-red-600 hover:text-white hover:scale-110"
                                    ...
                                    <div class="flex w-full items-center justify-between space-x-6 p-4 pb-3">
                                        <div class="flex-1 truncate">
                                            <div class="flex items-center space-x-3">
                                                ...
                                                <span v-if="file['edited'] === true" class="inline-block flex-shrink-0 rounded-full bg-blue-100 px-2 py-0.5 text-xs
                                             font-medium text-blue-700">Bearbeitet.</span>
                                            </div>
                                    ...
                                    <div>
                                        <div class="-mt-px flex divide-x divide-gray-200">
                                            <div class="flex w-0 flex-1">
                                                <CardActionButton :icon="TrashIcon" action-text="Löschen..." class="text-gray-900 hover:bg-red-600 hover:text-white" display-text="Löschen"
                                                                  @click="deleteFile(file.id)"/>
                                            </div>
                                            <div class="-ml-px flex w-0 flex-1">
                                                <NuxtLink v-if="file['state'] !== 'LOADING'" :to="`/org/${orgId}/ocr-import/sessions/${sessionId}/edit/${file.id}`"
        ...
        <div class="flex justify-between gap-2 mt-1 pr-2">
            ...
            <div>
                ...
                <SubmitButton v-else :icon="ExclamationTriangleIcon" class="bg-red-400 text-neutral-200" disabled @click="generateEntries">Keine validen Dokumente. </SubmitButton>
            </div>
        </div>
    </div>
</template>
...
/**
 * makes 31.03.2023 to 2023-03-31
 */
function reverseDatesAtDots(dottedDate) {
    if (dottedDate === null) return null;

    return dottedDate.split(".").join("-");
}

function sortFileList() {
    files.value = files.value.slice().sort((a, b) => reverseDatesAtDots(a.parsedTitle)?.localeCompare(reverseDatesAtDots(b.parsedTitle)) ?? files.value.length + 1);
}
